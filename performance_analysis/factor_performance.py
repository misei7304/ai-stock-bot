...
def print_factor_performance(title, empty_message, rows, bucket_function, bucket_order):
    ...
    for values in rows:
        *factor_values, current_return = values
        bucket = bucket_function(*factor_values)
    ...
    bucket_results = sorted(
        bucket_results,
        key=lambda x: bucket_order.get(x["bucket"], 999)
    )
...
def get_bollinger_bucket(bollinger_score):
    return f"볼린저점수 {bollinger_score}"
...
def analyze_rsi_performance():
    ...
    print_factor_performance(
        "RSI 구간별 실전 성과 분석",
        "분석 가능한 RSI 실전 성과 데이터가 없습니다.",
        rows,
        get_rsi_bucket,
        {
            "0~30": 1,
...
def analyze_macd_performance():
    ...
    cursor.execute("""
        SELECT r.macd, r.macd_signal, p.current_return
        FROM recommendation_performance p
        JOIN recommendations r
        ON p.recommendation_id = r.id
        WHERE p.current_return IS NOT NULL
...
def analyze_atr_performance():
    ...
    cursor.execute("""
        SELECT r.atr_percent, p.current_return
        FROM recommendation_performance p
        JOIN recommendations r
        ON p.recommendation_id = r.id
        WHERE p.current_return IS NOT NULL
    ...
    print_factor_performance(
        "ATR% 구간별 실전 성과 분석",
        "분석 가능한 ATR% 실전 성과 데이터가 없습니다.",
        rows,
        get_atr_percent_bucket,
        {
            "0~2%": 1,
            "2~4%": 2,
            "4~6%": 3,
...
def analyze_bollinger_performance():
    ...
    cursor.execute("""
        SELECT r.bollinger_score, p.current_return
        FROM recommendation_performance p
        JOIN recommendations r
        ON p.recommendation_id = r.id
        WHERE p.current_return IS NOT NULL
    ...
    print_factor_performance(
        "볼린저 점수별 실전 성과 분석",
        "분석 가능한 볼린저 실전 성과 데이터가 없습니다.",
        rows,
        get_bollinger_bucket,
        {}
    )
...
def analyze_final_score_performance():
    ...
    cursor.execute("""
        SELECT r.final_score, p.current_return
        FROM recommendation_performance p
        JOIN recommendations r
        ON p.recommendation_id = r.id
        WHERE p.current_return IS NOT NULL
    ...
    print_factor_performance(
        "최종점수 구간별 실전 성과 분석",
        "분석 가능한 최종점수 실전 성과 데이터가 없습니다.",
        rows,
        get_final_score_bucket,
        {
            "0점 미만": 1,
            "0~30": 2,
            "30~50": 3,
